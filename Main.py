...
import numpy as np 
import random
from Features.Open import open_app_or_website
...
def MainTaskExecution(Query):
    ...
    if ReturnData is not None:
        if isinstance(ReturnData, str):  # Check if 'data' is a string before using 'in' keyword
            if "open" in ReturnData:
                Value = open_app_or_website(TaskNew)
                return Value 